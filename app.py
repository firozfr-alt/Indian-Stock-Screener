...
def fetch_dossier_parallel(candidate, strategy):
    ...
    - Final Verdict: You MUST declare a definitive action: **[BUY]**, **[SELL]**, **[WATCH]**, or **[AVOID]**. 
    - Data Justification: Explicitly cite the P/E, ROE, Debt/Equity, or Cash Conv provided above to support your decision.
    - Recommended Holding Period: State the estimated optimal holding timeframe (e.g., 6-12 months, 1-3 years, 5+ years) to realize value, and provide a 1-sentence rationale.
    """
...
def run_four_agent_deep_audit(f_data):
    ...
    - Recommended Holding Period: State the estimated optimal holding timeframe (e.g., 6-12 months, 1-3 years, 5+ years) to realize value, and provide a 1-sentence rationale based on the thesis.
    """
...
# =========================================================
# 8. TRI-TAB + DEEP AUDIT TAB + AI DISCOVERY WORKFLOW
# =========================================================
tab_core, tab_smallcap, tab_penny, tab_fundamental, tab_discovery = st.tabs([
    "🏛️ Large & Mid-Cap Core", "🚀 Small-Caps", "⚠️ Penny & Micro-Caps", "🔬 Deep Fundamental AI Audit", "💡 AI Stock Discovery"
])
...
# =========================================================
# TAB 5: AI STOCK DISCOVERY (NEW)
# =========================================================
with tab_discovery:
    st.subheader("💡 AI Idea Generator & Dynamic Screener")
    st.caption("Tell the AI what kind of stocks you want to find. It will actively search the Indian market and run a deep pipeline scan on them.")

    search_query = st.text_input(
        "Describe your ideal stocks (e.g., 'High growth mid-caps in Defense', 'Debt-free small caps', 'Undervalued banks'):", 
        value="High growth mid-caps with low debt"
    )

    if st.button("🔍 Discover & Analyze", key="btn_discover"):
        if not ai_client:
            st.error("Google Gemini AI key is required for dynamic discovery.")
        else:
            with st.spinner("🧠 AI is searching the Indian market for matching tickers..."):
                discovery_prompt = f"""
                The user is looking for Indian stocks (NSE) matching this fundamental criteria: '{search_query}'. 
                Identify exactly 4 Indian companies that fit this description.
                OUTPUT FORMAT REQUIRED: Return ONLY a single line of comma-separated NSE ticker symbols. 
                No company names, no numbering, no .NS suffix. 
                Example output: DIXON,HAL,RELIANCE,TCS
                """
                
                try:
                    response = ai_client.models.generate_content(model="gemini-3.5-flash", contents=discovery_prompt)
                    raw_tickers = response.text.strip().split(',')
                    tickers_to_scan = [t.strip().upper().replace('.NS', '') for t in raw_tickers if t.strip()]
                    tickers_to_scan = tickers_to_scan[:4] # Enforce max 4 for speed
                except Exception as e:
                    tickers_to_scan = []
                    st.error(f"Failed to generate tickers: {e}")

            if not tickers_to_scan:
                st.error("AI failed to generate valid tickers. Try a different prompt.")
            else:
                st.success(f"✅ AI Selected the following targets: {', '.join(tickers_to_scan)}")
                
                with st.spinner("Step 1/2: Quant Scanning (Parallel Processing)..."):
                    tasks = [(t, "AI Dynamic Discovery", "core") for t in tickers_to_scan]
                ...
                if not df.empty:
                    df_sorted = df.sort_values(by=["Overall Score (/100)"], ascending=False).reset_index(drop=True)
                    st.dataframe(df_sorted[["Symbol", "Price (₹)", "Market Cap (Cr)", "P/E", "Debt/Equity", "ROE (%)", "Overall Score (/100)", "Tier"]], use_container_width=True)
                    top_picks = df_sorted.head(4).to_dict('records')
                    
                    st.markdown("### 🔬 AI Research Dossiers & Holding Targets")
                    dossier_map = {}
                    
                    with st.spinner("Step 2/2: Generating Deep Audits concurrently..."):
                        with concurrent.futures.ThreadPoolExecutor(max_workers=4) as ai_executor:
                            ai_futures = [ai_executor.submit(fetch_dossier_parallel, c, "core") for c in top_picks]
                            for future in concurrent.futures.as_completed(ai_futures):
                                sym, content = future.result()
                                dossier_map[sym] = content
                    ...
                    if dossier_map:
                        pdf = build_pdf_report(top_picks, dossier_map, "AI Discovery Research")
                        st.download_button("📄 Download AI Discovery PDF Report", data=pdf, file_name="AI_Discovery_Report.pdf", mime="application/pdf", key="dl_discovery")
